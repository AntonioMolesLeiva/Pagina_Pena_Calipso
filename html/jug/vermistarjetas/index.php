...
$(document).ready(function() {
	...
	$instruccion="SELECT p.fecha
				...
				WHERE idsancion IS NOT NULL AND i.jugador =  \"".$_SESSION['usu']."\"
				GROUP BY fecha
		...
		for ($i=0; $i<$nfilas; $i++){
			...
			if ($mes>=9) {
				if($ano!=$cAno) {
					...
					$instruccion1="SELECT p.fecha,p.marclocal,p.marcvisitante FROM partido p
				LEFT JOIN incidencia i ON p.fecha = i.fecha
				WHERE i.idsancion IS NOT NULL AND i.fecha>=\"".$ano."-09-01\" AND i.fecha<=\"".($ano+1)."-06-30\" AND i.jugador =  \"".$_SESSION['usu']."\" GROUP BY i.fecha
				ORDER BY p.fecha DESC";
				...
				if(($ano-1)!=$cAno) {
					...
					$instruccion1="SELECT p.fecha,p.marclocal,p.marcvisitante FROM partido p
				LEFT JOIN incidencia i ON p.fecha = i.fecha
				WHERE i.idsancion IS NOT NULL AND i.fecha>=\"".($ano-1)."-09-01\" AND i.fecha<=\"".$ano."-06-30\" AND i.jugador =  \"".$_SESSION['usu']."\" GROUP BY i.fecha
				ORDER BY p.fecha DESC";
					$consulta1 = mysql_query ($instruccion1, $conexion)
								or die ("FALLO EN LA CONSULTA $instruccion1");
					$nfilas1 = mysql_num_rows ($consulta1);
...
<div id="infopartido" class="col-md-9 col-sm-9" style="min-height:536px;background-color:rgba(12,12,12,0.3);">
  ...
  <h1 style="text-align:center;">¡Enhorabuena!</h1>
	<p style="text-align:justify;">Aún no tienes ninguna tarjeta en ninguna temporada.</p>
</div>
<?php }
