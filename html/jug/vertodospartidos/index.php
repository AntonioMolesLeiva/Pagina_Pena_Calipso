		...
		for ($i=0; $i<$nfilas; $i++){
			...
			if ($mes>=9) {
				if($ano!=$cAno) {
					...
					";
					$instruccion1="SELECT fecha,marclocal,marcvisitante FROM partido WHERE fecha>=\"".$ano."-09-01\" AND fecha<=\"".($ano+1)."-06-30\" ORDER BY fecha DESC";
				...
				if(($ano-1)!=$cAno) {
					...
					";
					$instruccion1="SELECT fecha,marclocal,marcvisitante FROM partido WHERE fecha>=\"".($ano-1)."-09-01\" AND fecha<=\"".$ano."-06-30\" ORDER BY fecha DESC";
					$consulta1 = mysql_query ($instruccion1, $conexion)
								or die ("FALLO EN LA CONSULTA $instruccion1");
					$nfilas1 = mysql_num_rows ($consulta1);
					if ($nfilas1>0) {
